 ...
 * @author KirrNguyen
 */
public class BorrowManageForm extends javax.swing.JPanel {

    Statement stmt = null;
    ResultSet rs = null;
    DefaultTableModel model;
    BorrowList borrowlist = new BorrowList();
    public BorrowManageForm() {
        initComponents();
    ...
    private void initComponents() {
        ...
        jButton1.setText("Xác Nhận Cho Mượn");
        ...
        jButton2.setText("Xóa Đăng Kí Mượn");
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        try{
            ...
            if(borrowlist.check(username, bookname)){
                if(borrowlist.update(bookname)){
                    borrowlist.accept(username, bookname);
                    borrowlist.delete(username, bookname);
                    View();
                }
            }
        }catch(Exception ex){
    ...
    public void search(){
        ...
        try {
            stmt = conn.createStatement();
            for(String s : search){
            rs = stmt.executeQuery("select * from registerbook where "+s+" like '"+text+"'");
            while(rs.next()){
                    model.addRow(new Object[]{rs.getString(1),rs.getString(2)});
                }
            }
